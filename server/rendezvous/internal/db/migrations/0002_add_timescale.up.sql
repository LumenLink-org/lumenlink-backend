...
SELECT add_compression_policy('operator_metrics', INTERVAL '7 days', if_not_exists => TRUE);

-- Add retention policy (drop data older than 90 days)
-- Adjust retention period as needed
SELECT add_retention_policy('operator_metrics', INTERVAL '90 days', if_not_exists => TRUE);
...
SELECT add_continuous_aggregate_policy('operator_metrics_hourly',
...
SELECT add_continuous_aggregate_policy('operator_metrics_daily',
